...
test.beforeEach(async ({ page }) => {
  await page.route("https://gizmoanalytics.io/**", (route) =>
    route.fulfill({ body: "" }),
  );
  await page.addInitScript(() => {
    Object.assign(window, { auditEvents: [] });
    window.gizmo = (name, properties) => {
      (window as unknown as { auditEvents: unknown[] }).auditEvents.push({
        name,
        properties,
      });
    };
...
test("model landing page has instant audio, comparisons, working copy, and signup success tracking", async ({
  page,
  context,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Hear Kokoro-82M" }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Play Neutral", exact: true })
    .first()
    .click();
  await expect
    .poll(() =>
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        (
          window as unknown as { auditEvents: { name: string }[] }
        ).auditEvents.some((event) => event.name === "sample_play"),
      ),
    )
    .toBeTruthy();
    ...
    () =>
      (
        window as unknown as {
          auditEvents: { name: string; properties: unknown }[];
        }
      ).auditEvents,
  );
...
test("comparison stops cross-script playback and skips missing samples", async ({
  page,
}) => {
  await page.goto("/compare?ids=megatts3,kokoro-82m,chatterbox");
  const playAll = page.getByRole("button", { name: "Play all back-to-back" });
  await playAll.first().click();
  await expect(
    page.getByRole("button", { name: "Pause Kokoro-82M neutral" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Play Chatterbox emotional" }).click();
  await expect(
    page.getByRole("button", { name: "Play Kokoro-82M neutral" }),
  ).toBeVisible();
  await expect
    .poll(() =>
      page.evaluate(
    ...
    .toBe(1);
  await page
    .getByRole("button", { name: "Pause Chatterbox emotional" })
    .click();
  await expect
    .poll(() =>
      page.evaluate(
...
test("directory filters work on mobile without hiding the results behind a long form", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("/directory");
  await expect(page.getByLabel("Search models")).toBeVisible();
  await page.getByLabel("Search models").fill("Chatterbox Turbo");
  await expect(
    page.getByRole("heading", { name: "Chatterbox Turbo", exact: true }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Filter models +" }).click();
  await expect(page.getByLabel("With audio samples")).toBeVisible();
  await page.getByLabel("Search models").fill("not-a-model");
  await page.getByRole("button", { name: "Clear all filters" }).click();
  await expect(page.getByLabel("Search models")).toHaveValue("");
});
test("named comparison has its own canonical and carries the chosen models into custom text", async ({
  page,
}) => {
  await page.goto("/compare/orpheus-vs-chatterbox-turbo");
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
    "href",
    "https://www.openspeech.dev/compare/orpheus-vs-chatterbox-turbo",
  );
